...
#
# WHAT IS STAMPED: not each file, but a dated MANIFEST of the five Letters —
...
printf '# sha256  path\n'                                  >> "$MANIFEST.tmp"
# ⛔⛔ SCOPED TO THE LETTERS, AND THE SCOPE IS THE WHOLE POINT. Everywhere else
# this script manifests `git ls-files` — the entire repository — which is right in
# a corpus repo where every tracked file IS a document. This repo is a WEBSITE that
# also holds five documents, and its own TIMESTAMPS.md rules on exactly this:
#
#   "Not stamped: every other page in this repository. Those are a website, not
#    documents, and stamping them would make the proof set meaningless by making
#    it universal."
#
# An unscoped manifest here would do precisely that. ⚠️ `letters/index.html` is the
# contents page and is excluded for the same reason the OTS leg excludes it.
# ⭐ The manifests still form a hash chain: each one covers all earlier manifests
# and tokens under timestamps/, which the second pattern below re-admits.
git ls-files -z 'letters/*.html' "$MDIR" \
  | tr '\0' '\n' \
  | grep -v '^letters/index\.html$' \
  | grep -v "^$MDIR/$DAY\." \
  | sort \
